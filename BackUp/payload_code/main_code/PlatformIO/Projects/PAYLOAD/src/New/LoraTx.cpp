...
bool LoraTx::begin() {
  ...
  // switch control lines (mislabelled DIO1/DIO2). Empirically (env:
  // test_lora_rfswitch_probe) CXT=LOW, CrX=HIGH is the only state that routes
  // the PA to the antenna; floating works but is unstable, the other driven
  // states kill the link entirely. The payload only transmits, so we hold this
  // state for the whole flight rather than toggling per packet.
  pinMode(PIN_LORA_DIO1, OUTPUT); digitalWrite(PIN_LORA_DIO1, LOW);   // CXT
  pinMode(PIN_LORA_DIO2, OUTPUT); digitalWrite(PIN_LORA_DIO2, HIGH);  // CrX
