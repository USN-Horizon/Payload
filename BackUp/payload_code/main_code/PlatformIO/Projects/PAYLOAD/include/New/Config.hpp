...
// exactly or it will not decode. Long-range profile = SF11, CR 4/8, BW 812.5 kHz.
...
//  Timing.
// -----------------------------------------------------------------------------
constexpr uint32_t TX_CYCLE_INTERVAL_MS    = 2000;  // One TX cycle / 2 s (0.5 Hz).
                                                    // At SF11 each packet is ~180 ms on
                                                    // air, so a full ~8-packet cycle is
                                                    // ~1.5 s. 2 s keeps duty ~73% with a
                                                    // little margin for retries. Raise this
                                                    // (e.g. 2500) if you see drops climbing.
