// Component test: BMP390 Barometer
//
...
void setup() {
    ...
    Serial.println();
    Serial.println("========================================");
    Serial.println("  COMPONENT TEST: BMP390 Barometer");
    Serial.println("========================================");
    ...
    result("BMP390 begin_I2C() – I2C comms + chip ID", initOk);

    if (!initOk) {
        Serial.println("  [HINT] Try changing BMP_ADDR to 0x76 (SDO pin low).");
        Serial.println("         Check: SDA=7, SCL=15, power to module.");
        goto done;
    }
