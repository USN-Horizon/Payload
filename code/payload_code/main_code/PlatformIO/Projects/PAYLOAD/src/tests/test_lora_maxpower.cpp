...
//
// Holds the antenna switch in the true TX state (CXT=HIGH, CrX=LOW, from the
// PA-on test_lora_rfswitch_burst run) and the DAC at full scale, then steps
// through the remaining power levers while transmitting steadily. Read RSSI on your MC for
...
// ---- DAC43401 (PA bias, held at full scale; 0x48 is the one that matters) ---
...
static constexpr uint32_t GAP_MS   = 10000;  // silent gap so RSSI windows are
                                             // unambiguous on the RX side.
...
void setup() {
    ...
    Serial.println("  [INFO] Antenna switch: CXT=HIGH CrX=LOW (TX path). DAC=full.");
    ...
    for (int e = 0; e < N_EXP; ++e) {
        ...
        while (millis() < tEnd) {
            ...
            int len = snprintf(msg, sizeof(msg), "MAXP E%d dbm=%d seq=%lu",
                               e + 1, EXPS[e].dbm, static_cast<unsigned long>(seq));
        ...
        Serial.printf("  ... %lu s radio silence before the next window ...\n",
                      static_cast<unsigned long>(GAP_MS / 1000));
        delay(GAP_MS);
    }
}
