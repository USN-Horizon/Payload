...
private:
  size_t buildMavlinkFrame(uint8_t* buf, int geigerCount, uint32_t tMs);
  void processSerialCommands();

  uint32_t lastGeigerTickMs_ = 0;
  uint32_t lastSensorSampleMs_ = 0;
  uint32_t loRaRetries_ = 0;
  uint32_t storageFails_ = 0;

  TelemetryRingBuffer loraBuf_;
  TelemetryPacket pendingPacket_;
