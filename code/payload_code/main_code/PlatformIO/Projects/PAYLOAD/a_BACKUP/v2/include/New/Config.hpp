...
// -----------------------------------------------------------------------------
constexpr float   LORA_FREQ_MHZ = 2450.0f;   // Carrier frequency in MHz.
                                             // WAS 2400.0, which is the exact
                                             // bottom of the SX1280 synthesizer
                                             // range (DS Table 3-3: 2400-2500)
                                             // AND the bottom of the 2.4 GHz ISM
                                             // band. With BW 812.5 the occupied
                                             // spectrum was ~2399.6-2400.4 MHz,
                                             // i.e. half of it below both limits
                                             // — unspecified PLL behaviour, any
                                             // band-select filter skirt eating
                                             // signal, and a regulatory problem.
                                             // 2450 is mid-band: best case for
                                             // the reference-design match and
                                             // far from both edges. If the
                                             // launch site turns out to be
                                             // Wi-Fi heavy, 2478 sits above
                                             // Wi-Fi ch11 (ends 2473) and below
                                             // the 2483.5 band edge.
constexpr float   LORA_BW_KHZ   = 812.5f;    // Bandwidth in kHz. SX1280 accepts
                                             ...
                                             // never starts.
                                             // Dropping to 203.125 is worth only
                                             // +4 dB (DS Table 6-1, SF12:
                                             // -126 -> -130 dBm), NOT the
                                             // "+10-12 dB" claimed in
                                             // Text/lora_rf_debug.md §7 — and it
                                             // costs 4x time-on-air AND collapses
                                             // the link's total frequency budget
                                             // from +/-50 ppm to +/-21 ppm
                                             // (DS Table 6-3). Do not go there
                                             // without a TCXO or a crystal spec
                                             // verified over -40..+85 C.
constexpr uint8_t LORA_SF       = 12;        // Spreading factor. SF11 -> SF12 buys
                                             // +3 dB sensitivity (DS Table 6-1 at
                                             // BW 812.5: -123 -> -126 dBm) for 2x
                                             // time-on-air. Cheapest real dB on
                                             // the payload side. Frequency
                                             // tolerance at BW 800/SF12 is still
                                             // +/-50 ppm (DS Table 6-3), so a
                                             // plain crystal is fine.
constexpr uint8_t LORA_CR       = 8;         // Coding rate denominator (8 = 4/8, max FEC).
                                             // Already maxed; per DS §6.2.4 FEC
                                             // does not improve thermal-noise
                                             // sensitivity, so there is nothing
                                             // further to win here.
constexpr int8_t  LORA_TX_PWR   = 13;        // Output power in dBm. 13 is the max
                                             // SetTxParams accepts (DS §11.7.4:
                                             // Pout = -18 + power, power <= 31);
                                             // true max radiated is +12.5 dBm
                                             // (DS §4.1 / Table 3-8).
                                             // The external PA saturates — +6 and
                                             // +13 radiated the same in the
                                             // maxpower test (2026-07-06) — so
                                             // this buys no measured dB today,
                                             // but it is free and keeps headroom
                                             // if the PA knee drifts with temp.
                                             // Actual output is set by the DAC43401
                                             // PA bias (~25 dB of range, held at
                                             // full scale in Sensors::begin()).
// NOTE: the ground-station receiver MUST match SF / CR / BW / freq / sync word
// exactly or it will not decode. Flight profile = 2450 MHz, SF12, CR 4/8,
// BW 812.5 kHz, sync 0x12, CRC on. (Whitening is NOT a LoRa parameter — see
// DS Table 11-60 — so there is nothing to match there.)
...
constexpr uint32_t TX_CYCLE_INTERVAL_MS    = 10000; // One TX cycle / 10 s.
                                                    // Time-on-air per DS §7.4.4.1 at the
                                                    // flight profile (SF12, BW 812.5,
                                                    // CR 4/8, explicit header, CRC on,
                                                    // 12-symbol preamble, ~36-byte frame):
                                                    //   Nsymbol = 12 + 4.25 + 8
                                                    //           + ceil(284/40)*8 = 88.25
                                                    //   Ts      = 2^12 / 812.5 kHz = 5.04 ms
                                                    //   ToA     = ~445 ms per packet
                                                    // A full cycle is <= 9 packets (8 sensors
                                                    // + heartbeat) = ~4.0 s, so duty is ~40%,
                                                    // leaving ~6 s for retries. If you need
                                                    // faster telemetry, drop packets from the
                                                    // cycle before shrinking this interval.
                                                    // (Was ~222 ms/packet at SF11; SF12
                                                    // doubled it to buy +3 dB.)
