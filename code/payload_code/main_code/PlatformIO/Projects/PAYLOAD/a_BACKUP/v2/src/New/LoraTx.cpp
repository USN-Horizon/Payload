...
bool LoraTx::begin() {
  ...
  // ORDER MATTERS. Every one of setBandwidth / setCodingRate / setSpreadingFactor
  // issues its own SetModulationParams, but the SX1280 datasheet (§14.4.1) says
  // that AFTER SetModulationParams you must write the SF-dependent value to
  // register 0x925 and set bit 0 of 0x93C. RadioLib only does those two writes
  // inside setSpreadingFactor(), so setSpreadingFactor() must come LAST or the
  // registers are left stale relative to the final modulation params.
  ...
  if (radio_.setSpreadingFactor(LORA_SF)        != RADIOLIB_ERR_NONE) return false;  // last!
  if (radio_.setOutputPower(LORA_TX_PWR)        != RADIOLIB_ERR_NONE) return false;

  // setCRC() takes a LENGTH IN BYTES, not a bool. In LoRa mode RadioLib accepts
  // only 0 (off) or 2 (on, = LORA_CRC_ENABLE 0x20, DS Table 14-53); anything
  // else returns RADIOLIB_ERR_INVALID_CRC_CONFIGURATION and silently leaves the
  // packet params untouched. setCRC(true) == setCRC(1) was doing exactly that —
  // CRC was on only because RadioLib's begin() happens to default it on.
  if (radio_.setCRC(2) != RADIOLIB_ERR_NONE) {
#if USE_SERIAL
    Serial.println("[LoRa] setCRC(2) failed");
#endif
    return false;
  }

  // NOTE: there is deliberately no setWhitening() call here. Whitening is not a
  // LoRa packet parameter — DS Table 11-60 gives LoRa exactly five: PreambleLength,
  // HeaderType, PayloadLength, CRC, InvertIQ. It exists only for GFSK/BLE/FLRC.
  // RadioLib agrees and returns RADIOLIB_ERR_WRONG_MODEM, so the old
  // setWhitening(true) was a no-op whose return value was discarded.
