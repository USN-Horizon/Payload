...
static int16_t imuRawToMrad(int16_t raw) {
  ...
  // 64-bit intermediate: raw * 2000 * 17453 overflows int32 for |raw| > 61
  // (~3.7 dps), which silently corrupted every real gyro reading.
  return clamp16(static_cast<int32_t>(
      static_cast<int64_t>(raw) * IMU_GYRO_FSR_DPS * 17453 / (32768LL * 1000LL)));
}
...
bool Sensors::begin() {
  ...
  if (dac43401Ok_) {
    ...
    delay(2);
    Wire.beginTransmission(I2C_ADDR_DAC43401);
    Wire.write(static_cast<uint8_t>(0xD1));  // GENERAL_CONFIG (NOT 0x09 — that
    Wire.write(static_cast<uint8_t>(0x00));  // bug left the DAC powered down
    Wire.write(static_cast<uint8_t>(0x00));  // and the PA unbiased)
    Wire.endTransmission();
    delay(2);

    // Full scale: the DAC biases the external PA stage (~25 dB of range,
    // measured -72 -> -47 dBm in test_lora_volt_sweep). Flight needs max.
    setLoraVolt(3300);
#if USE_SERIAL
