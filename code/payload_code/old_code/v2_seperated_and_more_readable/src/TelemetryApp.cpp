...
void TelemetryApp::tick() {
  ...
  TelemetrySample samp{now, geigerCount};
  loraBuf_.push(samp);
  ...
  uint8_t usedRetries = 0;
    ...
    if (loraOk_) {
      if (loraBuf_.size() >= 1) {
        ...
        String payload = buildLoraPacket(now, usedSamples);

        if (usedSamples == 0 || payload.length() == 0) {
          loraStatus = -1002;
          usedRetries = 0;
        } else {
...
String TelemetryApp::buildLoraPacket(uint32_t nowMs, size_t& usedSamples) {
  String payload;
  ...
  for (size_t i = 0; i < maxBatch; ++i) {
    ...
    if (!appendMeasurementRecord(payload, MEAS_ID_GEIGER_C1S, s.t_ms, s.geigerCount)) {
      break;
    }
    usedSamples++;
  }

  appendMeasurementRecord(payload, MEAS_ID_BARO_PA, nowMs, lastSens_.p_Pa);
  appendMeasurementRecord(payload, MEAS_ID_BARO_TC_CENTI, nowMs, lastSens_.t_cC);
  appendMeasurementRecord(payload, MEAS_ID_IMU_AX_MG, nowMs, lastSens_.ax_mg);
  appendMeasurementRecord(payload, MEAS_ID_IMU_AY_MG, nowMs, lastSens_.ay_mg);
  appendMeasurementRecord(payload, MEAS_ID_IMU_AZ_MG, nowMs, lastSens_.az_mg);
  appendMeasurementRecord(payload, MEAS_ID_IMU_GX_MDPS, nowMs, lastSens_.gx_mdps);
  appendMeasurementRecord(payload, MEAS_ID_IMU_GY_MDPS, nowMs, lastSens_.gy_mdps);
  appendMeasurementRecord(payload, MEAS_ID_IMU_GZ_MDPS, nowMs, lastSens_.gz_mdps);
  appendMeasurementRecord(payload, MEAS_ID_MAG_MX_UT10, nowMs, lastSens_.mx_uT10);
  appendMeasurementRecord(payload, MEAS_ID_MAG_MY_UT10, nowMs, lastSens_.my_uT10);
  appendMeasurementRecord(payload, MEAS_ID_MAG_MZ_UT10, nowMs, lastSens_.mz_uT10);
  appendMeasurementRecord(payload, MEAS_ID_SENSOR_OK_MASK, nowMs, lastSens_.okMask);

  appendMeasurementRecord(payload, MEAS_ID_LORA_RETRY_TOT, nowMs, loRaRetries_);
  appendMeasurementRecord(payload, MEAS_ID_STORAGE_FAIL_TOT, nowMs, storageFails_);
