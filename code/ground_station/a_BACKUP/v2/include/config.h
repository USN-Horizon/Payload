...
namespace config {
    ...
    // Flight profile: 2450 MHz, SF12, CR 4/8, BW 812.5 kHz, sync 0x12, CRC on.
    //
    // 2400.0 was the exact bottom of BOTH the SX1280 synthesizer range
    // (DS Table 3-3: 2400-2500 MHz) and the 2.4 GHz ISM band; with BW 812.5 kHz
    // half the occupied spectrum sat below both limits. 2450 is mid-band.
    // SF11 -> SF12 buys +3 dB sensitivity (DS Table 6-1 at BW 812.5:
    // -123 -> -126 dBm) at the cost of 2x time-on-air on the payload side.
    constexpr float RADIO_FREQUENCY_MHZ = 2450.0f;
    constexpr float RADIO_BANDWIDTH_KHZ = 812.5f;
    constexpr uint8_t RADIO_SPREADING_FACTOR = 12;
